...
def run_exp(name, epochs, steps, width, depth, urdf, root, tip):
    ...
    print('---------------------------------------------------')
    print(f'Num joints in chain {num_joints}')
    print('---------------------------------------------------')
    ...
    dataset = tf.data.Dataset.from_generator(
        ...
        (tf.TensorShape([batch_size, 6]), tf.TensorShape([batch_size, num_joints])),
    )
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'{MODEL_PATH_PREFIX}/{args.name}.json'):
        raise Exception(f'Experiment {args.name} already exists')
